...
const runMockTestPlannerAgent = async (input = {}) => {
  const prompt = buildPromptBundle(AI_STAGES.MOCK_TEST_PLANNER, input);
  const result = await runGeminiStage({ 
    stage: AI_STAGES.MOCK_TEST_PLANNER, 
    ...prompt, 
  ...
  const output = validateAgentPayload(parseStructuredJson(result.text), 'Mock test plan');
